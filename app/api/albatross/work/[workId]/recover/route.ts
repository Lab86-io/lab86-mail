...
export async function POST(req: NextRequest, context: { params: Promise<{ workId: string }> }) {
  try {
    ...
    await convexMutation((api as any).albatrossWorkV2.recordLapse, {
      userId: user.userId,
      workId,
      stepKey: step?.key,
    ...
    if (recovery === 'done') {
      const completed = await completeWorkStep({
        userId: user.userId,
    ...
    if (recoveryState === 'waiting' || recoveryState === 'paused') {
      await convexMutation((api as any).albatrossWorkV2.updateWorkState, {
        userId: user.userId,
        workId,
        state: recoveryState,
      });
    ...
    if (recoveryState === 'released') {
      await convexMutation((api as any).albatrossWorkV2.releaseWork, {
        userId: user.userId,
        workId,
