...
export function createToolPost(deps = dependencies) {
  return async function POST(req: NextRequest, ctx: { params: Promise<{ name: string }> }) {
    const { name } = await ctx.params;
    const tool = deps.tool(name);
    ...
    try {
      ...
      if (tool.name !== 'generate_daily_report')
        await deps.rate({
          userId: user.userId,
          key: `tool:${tool.name}`,
      ...
      const result = await deps.invoke(tool, body, {
        agent,
...
export const POST = createToolPost();
