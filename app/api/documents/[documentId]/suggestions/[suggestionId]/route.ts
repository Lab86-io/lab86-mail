...
export function createDocumentSuggestionPost(deps = defaultDependencies) {
  return async function POST(
    ...
    try {
      ...
      if (input.decision === 'dismiss') {
        const result = await deps.resolveDocumentSuggestion({
          userId: user.userId,
          documentId,
          suggestionId,
          status: 'dismissed',
      ...
      if (suggestion.baseRevision !== document.currentRevision) {
        return NextResponse.json(
          {
            ok: false,
            error: 'The document changed. Request a fresh suggestion for this revision.',
            code: 'CONFLICT',
          },
          { status: 409 },
        );
      }
      const result = await deps.applyDocumentSuggestion({
        userId: user.userId,
        documentId,
        suggestionId,
        ...
        model:
          suggestion.proposedModel.kind === 'sheet-changes'
            ? await deps.applySpreadsheetChanges(document.model, suggestion.proposedModel)
            : undefined,
      });
      if (!result.ok) {
...
export const POST = createDocumentSuggestionPost();
