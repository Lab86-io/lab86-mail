...
const Tool = ({ toolPart, defaultOpen = false, className }: ToolProps) => {
  ...
  const getStateBadge = () => {
    ...
    switch (state) {
      case 'input-streaming':
        return (
          <span className={cn(baseClasses, 'bg-[var(--color-success-soft)] text-[var(--color-success)]')}>
            Processing
          </span>
        );
      case 'input-available':
        return (
          <span className={cn(baseClasses, 'bg-[var(--color-warning-soft)] text-[var(--color-warning)]')}>
            Ready
          </span>
        );
      case 'output-available':
      ...
      case 'output-error':
        return (
          <span className={cn(baseClasses, 'bg-[var(--color-danger-soft)] text-[var(--color-danger)]')}>
            Error
          </span>
        );
      default:
        return (
          <span className={cn(baseClasses, 'bg-[var(--color-neutral-soft)] text-[var(--color-text-muted)]')}>
            Pending
          </span>
        );
    }
