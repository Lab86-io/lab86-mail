...
export const getEventsForWeek = (events: IEvent[], date: Date): IEvent[] => {
  ...
  return events.filter((event) => {
    ...
    const eventEnd = parseISO(event.endDate);
    return (
      isValid(eventStart) && isValid(eventEnd) && eventStart <= endOfWeekDate && eventEnd >= startOfWeekDate
    );
  });
};
...
export const getEventsForMonth = (events: IEvent[], date: Date): IEvent[] => {
  ...
  return events.filter((event) => {
    ...
    const eventEnd = parseISO(event.endDate);
    return (
      isValid(eventStart) && isValid(eventEnd) && eventStart <= endOfMonthDate && eventEnd >= startOfMonthDate
    );
  });
};
...
export const getEventsForYear = (events: IEvent[], date: Date): IEvent[] => {
  ...
  return events.filter((event) => {
    ...
    const eventEnd = parseISO(event.endDate);
    return (
      isValid(eventStart) && isValid(eventEnd) && eventStart <= endOfYearDate && eventEnd >= startOfYearDate
    );
  });
};
