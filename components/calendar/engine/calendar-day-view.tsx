...
import { DayViewMultiDayEventsRow } from '@/components/calendar/engine/day-view-multi-day-events-row';
...
export function CalendarDayView({ singleDayEvents, multiDayEvents }: IProps) {
  const { selectedDate, setSelectedDate, users, use24HourFormat, hourHeight, setHourHeight } = useCalendar();
  ...
  return (
    <div className="flex h-full min-h-0">
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <ScrollArea className="min-h-0 flex-1 overscroll-contain" type="always" ref={scrollAreaRef}>
          <div className="flex">
            {/* Hours column */}
          ...
          {currentEvents.length > 0 ? (
            <div className="flex items-start gap-2 px-4 pt-4">
              <span className="relative mt-[5px] flex size-2.5">
                <span className="absolute inline-flex size-full animate-ping rounded-ui bg-green-400 opacity-75"></span>
                <span className="relative inline-flex size-2.5 rounded-ui bg-green-600"></span>
              </span>
