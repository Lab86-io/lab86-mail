      ...
      .then((convertedFiles: FileUIPart[]) => {
        try {
          ...
          if (result instanceof Promise) {
            result
              .then(() => {
                // Reset only after success so the text survives a failed send
                if (!usingProvider) {
                  form.reset();
                }
                clear();
...
export const PromptInputSelectTrigger = ({ className, ...props }: PromptInputSelectTriggerProps) => (
  <SelectTrigger
    className={cn(
      'border-none bg-[var(--color-transparent)] font-medium text-muted-foreground shadow-none transition-colors',
