...
export const MessageBranchPage = ({ className, ...props }: MessageBranchPageProps) => {
  ...
  return (
    <ButtonGroupText
      className={cn('border-none bg-[var(--color-transparent)] text-muted-foreground shadow-none', className)}
      {...props}
    >
      ...
      {isImage ? (
        <>
          ...
          {onRemove && (
            <Button
              ...
              className="absolute top-2 right-2 size-6 rounded-ui bg-background/80 p-0 opacity-0 backdrop-blur-sm transition-opacity hover:bg-background group-hover:opacity-100 [&>svg]:size-3"
              onClick={(e) => {
                e.stopPropagation();
                onRemove();
              }}
        ...
        <>
          ...
          {onRemove && (
            <Button
              aria-label="Remove attachment"
              className="size-6 shrink-0 rounded-ui p-0 opacity-0 transition-opacity hover:bg-accent group-hover:opacity-100 [&>svg]:size-3"
              onClick={(e) => {
                e.stopPropagation();
                onRemove();
              }}
