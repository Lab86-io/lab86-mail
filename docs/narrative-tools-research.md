...
- Outgoing drafts use only explicitly selected observations. Private unrelated history is not silently put into a recipient's draft.
...
- `bun run test:coverage`: 3,270 tests passed; no failures. Every changed narrative library file meets or exceeds the staging baseline. Shared context retrieval: 100% line coverage; meeting prep: 88.24%.
