...
The iframe posts its access token in a form body. Save includes a unique receipt ID in Collabora's `ExtendedData`; WOPI stores it atomically with the revision. The UI polls for that receipt before allowing Google replacement. Google opening retries up to three exports if conversion changes the source version; continued changes produce a retryable error. Google saving uses a version check plus Drive v2 `If-Match` conditional media conversion. It never silently retries a conflicting write. Google working copies keep one identity across provider revisions. Changed Google content refreshes only a clean, unlocked copy and revokes stale editors. Completed Google writes retain their refreshed session in private recovery metadata if a concurrent update prevents immediate linking; the next open/save reconciles that metadata without repeating the provider write.

## Verification

On September 11, the independent staging service passed browser editing, explicit Save, durable receipt and stored-content verification for DOCX, XLSX and PPTX. The synthetic files were removed after verification.

Run focused tests with `bun test tests/collabora.test.ts tests/google-office.test.ts tests/chat-document-dogfood.test.ts tests/office-documents-convex-runtime.test.ts tests/documents-services.test.ts tests/proxy-basic-auth.test.ts`.
