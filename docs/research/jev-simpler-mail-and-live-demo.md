...
Focused route tests cover authenticated scope, production questions, strict inputs, a 32 KiB streaming body limit (including absent or dishonest Content-Length), rate limiting before body parsing, no model override, rate limits, sanitized failures, usage reporting, Brief preferences, source evidence and resolved obligations. Navigation tests cover category/folder precedence and custom views. The complete suite passed **4,222 tests**, with one existing skip and no failures; the two new library modules have 100% line coverage. Typecheck passed. Lint has only the repository's existing warning/information diagnostics.

`scripts/verify-jev-demo.mjs` exercised all detailed views and folders, active selection, mobile fit, failure recovery and missing-setup state at 390/768/1440 pixels, with no page errors. The four live provider examples ran at 1440 pixels. A loopback preview uses the actual components; its settings transport is synthetic. This is not a claim of signed-in staging browser verification. Release uses the existing staging workflow and subsequent deployment/API smoke checks.
