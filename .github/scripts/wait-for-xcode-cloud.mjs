...
async function preserveLogBundles(artifacts) {
  ...
  if (logBundles.length === 0) {
    console.warn(
      "Xcode Cloud returned no downloadable log bundle for the failed archive.",
    );
    return;
  }
  ...
  for (const { attributes } of logBundles) {
    ...
    const fileName = basename(
      attributes.fileName || "xcode-cloud.logbundle.zip",
    );
    writeFileSync(
      join(diagnosticsDirectory, fileName),
...
const testFlightSucceeded =
  testFlightAction?.attributes.completionStatus === "SUCCEEDED";
