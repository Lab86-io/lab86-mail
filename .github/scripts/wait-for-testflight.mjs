...
while (Date.now() < deadline) {
  ...
  if (!build) {
    console.log(
      `Waiting for TestFlight to register build ${process.env.BUILD_NUMBER}...`,
    );
  } else {
...
await appStoreConnect(
  `/v1/betaGroups/${process.env.TESTFLIGHT_GROUP_ID}/relationships/builds`,
  {
    method: "POST",
    body: JSON.stringify({
