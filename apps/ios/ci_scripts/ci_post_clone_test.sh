...
expected_default="LAB86_INFO_API_BASE_URL = https:/\$()/mail.lab86.io
...
# Every distributed build must reject non-production endpoint overrides.
if run_post_clone 2>/dev/null; then
  echo 'The staging workflow must reject non-production runtime overrides.' >&2
  exit 1
fi
