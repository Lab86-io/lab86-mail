...
afterAll(() => {
  dom.window.close();
  restoreGlobals();
  expect('IS_REACT_ACT_ENVIRONMENT' in globalThis).toBe(saved.get('IS_REACT_ACT_ENVIRONMENT') !== undefined);
...
describe('assistant layout resolution', () => {
  ...
  test('a split that cannot honor both minimums paints full instead of overflowing', () => {
    ...
    expect(
      resolveAssistantLayout({ open: true, presentation: 'split', width: ASSISTANT_SPLIT_MIN_WIDTH_PX - 1 }),
    ).toBe('full');
...
describe('escape and focus rules', () => {
  test('Escape closes only when nobody below claimed it and no IME is composing', () => {
    ...
    expect(escapeClosesAssistant({ key: 'Escape', defaultPrevented: false, isComposing: true })).toBe(false);
