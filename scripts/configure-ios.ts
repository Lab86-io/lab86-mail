...
writeFileSync(
  ...
  [
    ...
    `LAB86_INFO_API_BASE_URL = ${xcconfigValue(apiURL)}`,
    `LAB86_INFO_CLERK_PUBLISHABLE_KEY = ${clerkKey}`,
    `LAB86_INFO_CONVEX_DEPLOYMENT_URL = ${xcconfigValue(convexURL)}`,
    `LAB86_INFO_CLERK_FRONTEND_API_HOST = ${clerkHost}`,
    '',
  ].join('\n'),
  { mode: 0o600 },
);
