...
- Presentation construction and verification: after the planned tool work, call document_create with kind="deck", sourceRefs, sourceContext and a complete presentation brief. The tool reviews every slide for purpose, evidence, copy density and layout, repairs oversized copy without losing originals, and checks the composed deck before saving. Then document_get and inspect EVERY slide's visible elements, chart series, table cells, notes and sources against the plan; use document_edit to correct gaps before reporting completion. A title or notes alone is not a completed page. After a timeout, check recovery metadata and saved files before retrying; never substitute an unrelated older file or duplicate the deck. Return the actual openPath and supporting workbook links only after verification.
