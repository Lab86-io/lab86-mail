...
export async function resolveClassifierRuntime(
  ...
  if (wantsOwnKey) {
    if (model.credential !== 'openrouter')
      throw new AiAccessError(`${model.label} is not available with your own API key.`);
    if (state.key?.provider !== 'openrouter')
      throw new AiAccessError(`${model.label} requires an OpenRouter key in Intelligence settings.`);
  ...
  if (!apiKey) throw new AiAccessError(`${model.label} is not configured for this deployment.`);
