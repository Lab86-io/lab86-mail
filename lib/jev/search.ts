...
const searchDefaults = { loadJevPolicy, resolveClassifierRuntime, evaluateClassifier, recordClassifierUsage };
export async function rerankMail(
  ...
  const { loadJevPolicy, resolveClassifierRuntime, evaluateClassifier, recordClassifierUsage } = dependencies;
  ...
  try {
    ...
    const key = createHash('sha256')
      .update(JSON.stringify([runtime.model.id, JEV_QUESTION_VERSION, userId, query, candidates]))
      .digest('hex');
    ...
    if (cached && Date.now() - cached.at < 60_000) scores = cached.scores;
      ...
      const results = await mapConcurrent(chunks, 4, async (chunk) => {
        ...
        await recordClassifierUsage(runtime, 'jev_search', response);
